...
fake_items_db = {
    "asd21": {"title": "React Dev"},
    "bsd11": {"title": "React Dev 2"},
    "grd256": {"title": "Fast API Dev"},
}
...
async def get_board(board_id: str, db: Session = Depends(get_db)):
    ...
    if board is None:
        raise HTTPException(status_code=404, detail="Board {} not fount".format(board_id))
    return board


@router.post("/create/", response_model=schemas.Board)
async def create_board(board_id: str, board: schemas.BoardCreate, db: Session = Depends(get_db)):
    ...
    print(board)
    db_board = models.Board(id=board_id, title=board.title, describtion=board.describtion, url=board.url, company_name=board.company_name, logo_url=board.logo_url, created_at=datetime.now())
    db.add(db_board)
    db.commit()
    db.refresh(db_board)
    return db_board
